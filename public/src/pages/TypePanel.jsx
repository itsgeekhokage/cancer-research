...
const All = ({ data, setCancerList }) => {
  ...
  return (
    <div>
      <h3 className={styles.selectionPanelHeading}>{"Pie Chart showing the distribution of various kind of Cancers "}</h3>
...
const DistrictGraph = ({ data, typeOfTumor }) => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [rows, setRows] = useState([]);
  const RADIAN = Math.PI / 180;
  const renderCustomizedLabel = ({
  ...
  useEffect(() => {
    data = data?.filter((item) => item.typeOfTumor === typeOfTumor);
  ...
  return (
    <>
      <h3 className={styles.selectionPanelHeading}>
        {"Pie Chart showing the distribution of "} {typeOfTumor} {"across various districts"}
      </h3>
      <div className={styles.chartArea}>
        <PieChart
          width={500}
...
const GenderGraph = ({ data, typeOfTumor }) => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [rows, setRows] = useState([]);
  const RADIAN = Math.PI / 180;
  const renderCustomizedLabel = ({
  ...
  return (
    <div>
      <h3 className={styles.selectionPanelHeading}>
        {"Pie Chart showing the distribution of "} {typeOfTumor}{" "}
...
const AgeGraph = ({data, typeOfTumor}) => {
  const [activeIndex, setActiveIndex] = useState(null);
  const [rows, setRows] = useState([]);
  const RADIAN = Math.PI / 180;
  const renderCustomizedLabel = ({
  ...
  useEffect(() => {
    ...
    const { ten, twenty, thirty, forty, fifty, sixty, seventy, eighty } = fillist.reduce(
      (acc, age) => {
        if (age <= 10) {
          acc.ten++;
        } else if (age <= 20) {
      ...
      { ten: 0, twenty: 0, thirty : 0, forty : 0, fifty : 0, sixty: 0, seventy: 0, eighty: 0 }
    );
